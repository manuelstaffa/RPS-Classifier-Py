...
def main():
    ...
    with mp_hands.Hands(
        max_num_hands=config["mphands"].getint("max_num_hands"),
        model_complexity=config["mphands"].getint("model_complexity"),
        min_detection_confidence=config["mphands"].getfloat("min_detection_confidence"),
        min_tracking_confidence=config["mphands"].getfloat("min_tracking_confidence"),
    ) as hands:
        while capture.isOpened():
            ...
            detection_frequency = config["evaluate"].getfloat("detection_frequency")
            detection_time = config["evaluate"].getfloat("detection_time")
            movement_accuracy = config["evaluate"].getfloat("movement_accuracy")
            win_time = config["evaluate"].getfloat("win_time")
            ...
            if predicted:
                result_text_time = (
                    result_text + f" [{win_time-round(current_time - eval_time)}s]"
                )
